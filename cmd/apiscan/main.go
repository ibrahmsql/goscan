...
// printBanner displays the ASCII art banner for the apiscan tool.
// If printHelp is true, it also prints usage instructions and example commands.
...
// main is the entry point for the apiscan CLI tool, handling argument parsing, configuration, API scanning, result display, and output file generation.
// It validates required arguments, processes optional flags, initializes the scanner, runs the scan, prints color-coded results and statistics, and exports results to JSON files.
func main() {
	if len(os.Args) < 3 {
		printBanner(true)
		return
